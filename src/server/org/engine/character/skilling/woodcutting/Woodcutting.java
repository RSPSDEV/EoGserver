package server.org.engine.character.skilling.woodcutting;
...
import server.org.engine.character.content.randomevents.BirdsNest;
import server.org.engine.character.content.randomevents.TreeSpirit;
import server.org.engine.character.skilling.SkillHandler;
					...
					CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
						...
						c.turnPlayerTo(c.objectX, c.objectY);
							...
							if(c.playerSkillProp[8][6] > 0) {
								...
								BirdsNest.DropNest(c);
								TreeSpirit.PlaceSpirit(c);
							}
							deleteTime(c);
							if (!hasAxe(c)) {
	...
	private static int getRespawnTime(Client c, int treep) {
		...
		for(int i = 0; i < trees.length; i++) {
			if(treep == trees[5][i]) {
					...
					respawn = 15+Misc.random(15);	//9
			} else if(treep == trees[0][i]) {
					respawn = 10+Misc.random(15);	//6
			}
		}
		return respawn;
	}
