...
public class Slayer {
	...
	public int[] lowTasks = {1648, 1612, 117, 1265, 90, 103, 78, 119, 18, 101, 1265, 107};
	...
	public void giveTask2() {
		for (int j = 0; j < lowTasks.length; j++) {
			if (lowTasks[j] == c.slayerTask) {
				c.sendMessage("You already have an easy task, to kill " + c.taskAmount + " " + Server.npcHandler.getNpcListName(c.slayerTask).toLowerCase().replaceAll("_", " ") + "s.");
				return;		
			}	
		}
		giveTask(1);
	...
	public void giveTask(int taskLevel) {
		...
		c.sendMessage("You have been assigned to kill " + c.taskAmount + " " + Server.npcHandler.getNpcListName(given).toLowerCase().replaceAll("_", " ") + "s as a slayer task.");
	}
