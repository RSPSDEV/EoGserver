...
public class ItemAssistant {
	...
	**/
	...
	{4753,4980},{4755,4986},{4757,4992},{4759,4998},{4886,4890},{4887,4890},{4888,4890},{4889,4890}};
	...
	public void keepItem(int keepItem, boolean deleteItem) { 	
		ItemManager.keepItem(c, keepItem, deleteItem);
	}
	public void resetKeepItems() {
		ItemManager.resetKeepItems(c);
	}
	public void deleteAllItems() {	
		ItemManager.deleteAllItems(c);
	}
	public void dropAllItems() {
		ItemManager.dropAllItems(c);
	}
	...
	public boolean addItem(int item, int amount) {
		return ItemManager.addItem(c, item, amount);
	}
	...
	**/
	...
	public void getRequirements(String itemName, int itemId) {
		...
		if(itemName.contains("verac") || itemName.contains("guthan") || itemName.contains("dharok") || itemName.contains("torag")) {
			...
			} else if(itemName.contains("greataxe")) {
				c.attackLevelReq = 70;
				c.strengthLevelReq = 70;
	...
	**/
	public boolean is2handed(String itemName, int itemId) {
		if(
			itemName.contains("ahrim") || itemName.contains("karil") ||
			itemName.contains("verac") || itemName.contains("guthan") ||
			itemName.contains("dharok") || itemName.contains("torag") ||
			itemName.contains("100") || itemName.contains("75") ||
			itemName.contains("50") || itemName.contains("25")) {
			return true;
		}
	...
	public boolean wearItem(int wearID, int slot) {
		...
		synchronized(c) {
			...
			if(c.playerItems[slot] == (wearID+1)) {		
				...
				case 7372:
					if(c.dragonSlayer <= 6){
					c.sendMessage("You need to complete Dragon slayer to wear this.");
					return false;
					}
				break;
				}
				...
				if (wearID == 4722 && c.dhlegs >= 1){
					c.sendMessage("Please use your current Dharok's Platelegs first.");
					return false;
				}

				if (wearID == 4720 && c.dhbody >= 1){
					c.sendMessage("Please use your current Dharok's Platebody first.");
					return false;
				}
				if (wearID == 4718 && c.dhaxe >= 1){
					c.sendMessage("Please use your current Dharok's Greataxe first.");
					return false;
				}
				if (wearID == 4716 && c.dhelm >= 1){
					c.sendMessage("Please use your current Dharok's Helm first.");
					return false;
				}
				/*Slots:
				 	0 hat
